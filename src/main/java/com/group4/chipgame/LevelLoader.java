...
public class LevelLoader {
    ...
    public List<Collectible> loadCollectibles(String levelFilePath, LevelRenderer levelRenderer) throws IOException {
        ...
        if (!levelData.has("collectibles")) {
            return new ArrayList<>();
        }
        JSONArray collectiblesArray = levelData.getJSONArray("collectibles");

        List<Collectible> collectibles = new ArrayList<>();

        for (int i = 0; i < collectiblesArray.length(); i++) {
            JSONObject collectibleData = collectiblesArray.getJSONObject(i);
            String type = collectibleData.getString("type");
            int x = collectibleData.getInt("x");
            int y = collectibleData.getInt("y");

            Collectible collectible = collectibleCreators.getOrDefault(type, (a, b) -> null).apply(x, y);
            if (collectible != null) {
                collectibles.add(collectible);
                if (levelRenderer.getTiles()[y][x] != null) {
                    levelRenderer.getTiles()[y][x].setOccupiedBy(collectible);
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (tileType.startsWith(TRAP_PREFIX)) {
                    ...
                    for (String trapNumber : trapNumbers) {
                        if (trapNumber.equals(TRAP_PREFIX.replace("_", ""))) continue;
