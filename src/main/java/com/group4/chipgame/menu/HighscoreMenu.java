...
public class HighscoreMenu extends BaseMenu {
    ...
    public static void populateHighscoreMenu(VBox menuBox, Stage primaryStage, Main mainApp) {
    ...
    private static List<File> loadLevelFiles() throws IOException {
        Enumeration<URL> levelFiles = HighscoreMenu.class.getClassLoader().getResources("levels");
    ...
    private static void addNavigationButtons(VBox menuBox, Stage primaryStage, Main mainApp, int totalLevels) {
        if (currentPage > 0) {
            addButton("Previous", menuBox, primaryStage, mainApp, () -> {
                currentPage--;
                populateHighscoreMenu(menuBox, primaryStage, mainApp);
            });
        }
    ...
    private static void populateHighscoreList(VBox menuBox, Stage primaryStage, Main mainApp) {
        menuBox.getChildren().clear();
        HighscoreList.populateHighscoreList(menuBox, primaryStage, mainApp);
    }
}
